...
import { StyleSheet, Text, View, Image, TouchableOpacity, ActivityIndicator, ScrollView, TextInput, BackHandler, Alert, ImageBackground, Platform } from 'react-native';
...
export default function Login ( props ) {
  ...
  useEffect( (async) => {
    ...
    OneSignal.setAppId("8ccc9d23-79d5-49b1-9893-a63e09a6e7d4");
    //END OneSignal Init Code
  
    //Method for handling notifications opened
    OneSignal.setNotificationOpenedHandler(notification => {
      console.log("OneSignal: notification opened:", notification);
      console.log('xyajakja======================>',notification)
    })
  ...
  return (
    <ImageBackground style={styles.imageBackgroundWrapper} resizeMode='cover' source={require( '../../../assets/images/loginbackground.png' )} >
      <ScrollView>
        {( loading ) &&
        ...
        <View style={styles.logoWrapper}>
          <Image resizeMode='contain' source={LOGO_ALLADIN} style={styles.logoImage} />
        </View>
        <View>

          <View style={styles.emailWrapper}>
            <Image source={USER_PROFILE} resizeMode='contain' style={styles.emailImage} />
        ...
        <View style={styles.emailWrapper}>
          ...
          <Text style={styles.emailText}> {t('placeholders.auth.password')}</Text>
        ...
        <TouchableOpacity
          ...
          <Text style={styles.forgetText}> {t('placeholders.auth.forgotPassword')}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create( {
  ...
  logoWrapper: { alignItems: 'center', justifyContent: 'center', paddingTop: hp( 14 ) },
  logoImage: { height: wp( 50 ), width: wp( 50 ), },
  emailWrapper: { marginTop: hp( 3 ), marginHorizontal: wp( 5 ), flexDirection: 'row', alignItems: 'center' },
  emailImage: { height: hp( 3 ), width: wp( 8 ) },
  emailText: { fontSize: 13, fontFamily: MONTSERRAT_BOLD, color: '#000' },
  inputWrapper: { marginHorizontal: wp( 6 ), flexDirection: 'row', alignItems: 'center', elevation: 2, width: wp( 90 ), marginTop: hp( 1.5 ), backgroundColor: WHITE },
  input: { width: wp( 80 ), marginLeft: wp( 4 ), paddingVertical: Platform.OS === 'ios' ? hp( 2 ) : hp( 2 ) },
  forgetWrapper: { justifyContent: 'flex-end', marginRight: wp( 5 ), alignItems: 'flex-end', marginTop: hp( 2 ) },
  forgetText: { fontFamily: MONTSERRAT_REGULAR, fontSize: 15, color: '#9066e6' },
  loginWrapper: { paddingVertical: hp( 2 ), backgroundColor: '#9066e6', marginHorizontal: wp( 5 ), justifyContent: 'center', alignItems: 'center', marginTop: hp( 4 ) },
