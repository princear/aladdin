...
import { AllLangugae, ParticularLangugae } from "../../redux/Action/langugaeAction";
...
export default function LanguageScreen({ props, navigation }) {
    ...
    const radioClick = (id, code) => {
        ...
        setType(id)
        changeLanguage(code)
        dispatch(ParticularLangugae({ language_id: id }))
        // RNRestart.Restart();
    ...
    return (
        <View style={styles.container}>
            <View style={{ backgroundColor: '#9066e6', paddingVertical: hp(2), flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <TouchableOpacity onPress={() => navigation.goBack()}>
                        <Image resizeMode='contain' source={require('../../assets/images/left-arrow.png')} style={{ height: hp(3), width: wp(8), marginRight: wp(6), marginLeft: wp(2) }} />
                    </TouchableOpacity>
                    <Text style={{ fontSize: 14, color: '#fff', fontFamily: "Montserrat-Bold" }}>{t('placeholders.language.chose_language')}</Text>
                </View>
            ...
            <View style={{ marginHorizontal: wp(5) }}>
                <Image source={require('../../assets/images/translate.png')} style={styles.translateImage} resizeMode='contain' />
                <View>
                    <Text style={styles.headinglanguage}>{t('placeholders.language.prefered_language')}</Text>
                    <Text style={styles.subheadinglanugauge}>{t('placeholders.language.select_language')}</Text>

                    {
                        showLangugae ?.map((item) => {
                            return (
                                <View style={styles.langugaeWrapper}>
                                    <Text style={styles.languageText}>{item.language_name}({item.language_code})</Text>
                                    <View style={{ flexDirection: 'row' }}>
                                        <TouchableOpacity key={item.id}
                                            onPress={radioClick.bind(this, item.id, item.language_code)}>
                                            <View style={{
                                                height: 24,
                                                width: 24,
                                                borderRadius: 12,
